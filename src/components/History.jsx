...
import { X, Search, Download, ExternalLink, ChevronUp, ChevronDown } from 'lucide-react';

export default function History({ isOpen, onClose, history, onExportAfip }) {
  ...
  return (
    <div className="modal-overlay">
      <div className="modal" style={{ maxWidth: '1240px', width: '92vw', height: 'min(780px, 92vh)', display: 'flex', flexDirection: 'column' }}>
        ...
        {/* Modal Body / Table */}
        <div className="modal-body" style={{ flex: 1, minHeight: 0, padding: '20px' }}>
            ...
            <table>
              ...
              <tbody>
                {filteredHistory.length === 0 ? (
                  <tr>
                    <td colSpan="14" style={{ textAlign: 'center', padding: '30px', color: 'hsl(var(--text-muted))', fontSize: '13px' }}>
                    ...
                    return (
                      <tr key={item.invoiceId || idx}>
                        ...
                        <td style={{ textAlign: 'center' }}>
                          {item.isDemoMode ? (
                            <span style={{ fontSize: '11px', color: 'hsl(var(--text-muted))' }} title="Modo demo (sin conexión real)">Demo</span>
                          ) : (
                            <a 
                              href={`https://me.yiqi.com.ar/view/FACTURA_COMPRA?schemaId=1491&entityId=660&id=${item.invoiceId}#/${item.invoiceId}`}
                              target="_blank" 
